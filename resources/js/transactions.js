...
window.addEventListener('DOMContentLoaded', function () {
            ...
            {data: 'date'},
            {
                data: row => `
                    <div class="d-flex flex">
                        ...
                        <button type="submit" class="ms-2 btn btn-outline-primary edit-transaction-btn" data-id="${row.id}">
                            <i class="bi bi-pencil-fill"></i>
                        </button>
                        <button type="submit" class="ms-2 btn btn-outline-primary open-receipt-upload-btn" data-id="${row.id}">
    ...
    document.querySelector('#transactionsTable').addEventListener('click', function (event) {
        ...
        if (editBtn) {
            ...
            uploadReceiptModal._element.querySelector('.upload-receipt-btn').setAttribute('data-id', transactionId)
...
function openEditCategoryModal(modal, {id, ...data}) {
    for (let name in data) {
        const nameInput = modal._element.querySe
        lector(`[name="${name}"]`)
